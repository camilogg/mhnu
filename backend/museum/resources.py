...
class RecordModelResource(ModelResource):
    country_code = fields.Field(
        attribute="country__country_code", column_name="countryCode", readonly=True
    )
    ...
    def before_import_row(self, row, row_number=None, **kwargs):
        ...
        index = list(map(lambda x: x[0], list(row.items()))).index("taxonRemarks") + 1

        additional_data_columns = list(row.items())[index:]
        ...
        if row["occurrenceStatus"]:
            OccurrenceStatus.objects.get_or_create(name=row["occurrenceStatus"])
        if row["preparations"]:
        ...
        if row["samplingProtocol"]:
            SamplingProtocol.objects.get_or_create(name=row["samplingProtocol"])
        if row["habitat"]:
        ...
        if row["county"] and row["stateProvince"]:
            ...
            County.objects.get_or_create(name=row["county"], state_province=state)
        if row["municipality"]:
        ...
        if row["nomenclaturalCode"]:
            NomenclaturalCode.objects.get_or_create(name=row["nomenclaturalCode"])
