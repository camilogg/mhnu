...
occurrence_id_regex = RegexValidator(
    regex=r"(Unillanos|892000757-3):MHNU-[A-Z]{1,2}:\d{3,}", message=_("Invalid format")
)

catalog_number_regex = RegexValidator(regex=r"MHNU-[A-Z]{1,2}[ -]\d{3,}")
...
def validate_date(value):
    ...
    regex = (
        r"^(((20(([02468][048])|([13579][26]))-02-29(\/((0[1-9])|(1[0-9])"
        "|(2[0-9])|30|31))?))|(20[0-9]{2}(-(0[1-9]|1[0-2]))?)|(20[0-9][0"
        r"-9])-((((0[1-9])|(1[0-2]))-((0[1-9])|(1[0-9])|(2[0-8]))(\/((0[1"
        r"-9])|(1[0-9])|(2[0-9])|30|31))?)|((((0[13578])|(1[02]))-31(\/(("
        "0[1-9])|(1[0-9])|(2[0-9])|30|31))?)|(((0[13-9])|(1[0-2]))-(29|3"
        r"0)(\/((0[1-9])|(1[0-9])|(2[0-9])|30|31))?))))(\/(((20(([02468]["
    ...
    if not match:
        raise ValidationError(
            _(
                "Invalid date or date format, must be: YYYY-MM-DD, YYYY-MM-DD/DD"
    ...
    if len(value) > 7:
        ...
        if date and len(date_values) == 2:
            ...
            if len(date_values[1]) == 2:
                ...
                elif date > now or datetime(date.year, date.month, day).date() > now:
