...
class RecordAdmin(AutocompleteFilterMixin, DjangoObjectActions, TabbedModelAdmin):
    ...
    list_filter = tuple(map(lambda x: (x, AutocompleteListFilter), autocomplete_fields))
    ...
    def export(self, request, queryset):
        ...
        ej = ExportJob.objects.create(
            # app_label=app_label,
            # model=model_name,
            site_of_origin=request.scheme
            + "://"
            + request.get_host(),
        )

        url = reverse(
            f"admin:{ej._meta.app_label}_{ej._meta.model_name}_change", args=[ej.pk]
        )
        return redirect(url)
    ...
    pdf_detail.attrs = {"target": "_blank"}
