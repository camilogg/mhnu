...
const Collections = () => {
  ...
  const loadOptions = async (inputText, callback) => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_HOST}/${filter.value}?name=${inputText}&collection_code=${collection.value}`
    )
  ...
  const handleClickBtnSearch = async () => {
    setLoading(true)
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_HOST}/records?collection=${collection.value}&${
  ...
  const fetchData = useCallback(
    async ({ pageIndex, collection, filter, search }) => {
      if (collection && filter && search) {
        ...
        setLoading(true)
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_HOST}/records?collection=${
            collection.value
